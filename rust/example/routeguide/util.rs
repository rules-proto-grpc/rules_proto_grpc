...
pub fn load_db() -> Vec<Feature> {
    ...
    let features: Vec<serde_json::Value> = serde_json::from_str(data).unwrap();
    features.into_iter().map(|r: serde_json::Value| -> Feature {
        let mut f = Feature::new();
        f.set_name(r["name"].to_string());
        f.mut_location().set_latitude(r["location"]["latitude"].as_i64().unwrap() as i32);
        f.mut_location().set_longitude(r["location"]["longitude"].as_i64().unwrap() as i32);
        f
    }).collect()
}
