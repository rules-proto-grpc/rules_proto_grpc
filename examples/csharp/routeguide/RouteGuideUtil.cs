...
public static class RouteGuideUtil
{
    ...
    public static List<Feature> ParseFeatures(string filename)
    {
        ...
        var jsonFeatures = JsonDocument.Parse(File.ReadAllText(filename)).RootElement;

        foreach(var jsonFeature in jsonFeatures.EnumerateArray())
        {
            features.Add(new Feature{
                Name = jsonFeature.GetProperty("name").GetString(),
                Location = new Point {
                    Longitude = jsonFeature.GetProperty("location").GetProperty("longitude").GetInt32(),
                    Latitude = jsonFeature.GetProperty("location").GetProperty("latitude").GetInt32()
                }
            });
        }
        return features;
